...
def get_args():
    ...
    parser.add_argument("--word_association", type=str, default='/mnt/home/user24/project/Hallucination/hpred/AMBER0617/data/relation.json')
    parser.add_argument("--safe_words", type=str, default='/mnt/home/user24/project/Hallucination/hpred/AMBER0617/data/safe_words.txt')
    parser.add_argument("--inference_data", type=str)
    parser.add_argument("--annotation", type=str, default='/mnt/home/user24/project/Hallucination/hpred/AMBER0617/data/annotations.json')
    parser.add_argument("--metrics", type=str, default='/mnt/home/user24/project/Hallucination/hpred/AMBER0617/data/metrics.txt')
    parser.add_argument("--similarity_score", type=float, default=0.8)
    parser.add_argument('--evaluation_type', choices=['a', 'g', 'd', 'de', 'da', 'dr'], help='a: all tasks and dimensions    g: generative task    d: descriminative task    de, da, dr: existence, attribute, relation', default='g')
...
def main(args):
    ...
    try:
        ...
        inference_data = convert_jsonl_to_json(args.inference_data)
    ...
    for i in tqdm(range(len(inference_data))):
        ...
        if ground_truth[id-1]['type'] == 'generative':
            nouns = extract_nouns(inference_data[i]['text'].split('ASSISTANT: ')[-1])
            after_process_nouns = []
            for noun in nouns:
            ...
            response = 'Yes'
            response = 'Yes' if 'yes' in inference_data[i]['text'].lower() else response
            response = 'No' if 'no' in inference_data[i]['text'].lower() else response
